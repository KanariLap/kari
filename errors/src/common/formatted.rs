...
use kari_span::{source_map::SpanLocation, symbol::with_session_globals, Span};
...
pub struct Formatted {
    ...
    pub span: Span,
    /// The backtrace to track where the Kari error originated.
...
impl fmt::Display for Formatted {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if std::env::var("KARI_TESTFRAMEWORK").unwrap_or_default().trim().to_owned().is_empty() {
        ...
        let Kari_backtrace = std::env::var("KARI_BACKTRACE").unwrap_or_default().trim().to_owned();
        match Kari_backtrace.as_ref() {
            "1" => {
